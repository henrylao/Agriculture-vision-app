# Computer Vision for Agriculture on Android
...
If you don't have the PyTorch environment set up to run the script, you can download the model file `#` [here](#) to the `../proj_root/AgricultureVision/app/src/main/assets` folder, then skip the rest of this step and go to step 2 directly.
...
The Python script `export.py` in the `models` folder of the [MSCG repo](#) is used to generate a TorchScript-formatted YOLOv5 model named `#` for mobile apps.
...
[comment]: <> (python export.py --weights <yolov5s.pt> --include torchscript)
```
...
Start Android Studio, then open the project located in `../proj_root/AgricultureVision/`
